...
namespace GammaFour.DataModelGenerator.RestService.RestServiceClass
{
    ...
    public class PatchMethod : SyntaxElement
    {
        ...
        public PatchMethod(TableElement tableElement)
        {
            ...
            //        [HttpPatch]
            //        public async Task<IActionResult> PatchFungiblesAsync([FromBody] JArray jArray)
            //        {
            //            <Body>
            //        }
        ...
        private static SyntaxList<CatchClauseSyntax> CatchClauses
        {
            get
            {
                ...
                clauses.Add(
                    ...
                    .WithBlock(PatchMethod.HandleException));
        ...
        private BlockSyntax Body
        {
            get
            {
                ...
                //            }
                statements.Add(
                    SyntaxFactory.TryStatement(PatchMethod.CatchClauses)
                    .WithBlock(
        ...
        /// </summary>
        private SyntaxList<StatementSyntax> ResolveParentRecordBlock
        {
            get
            {
                // This is used to collect the statements.
                ...
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleCurrencyCodeKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleCusipKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleFigiKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleIsinKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleSedolKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleSymbolKey).ConfigureAwait(false);
                //                    await lockingTransaction.WaitReaderAsync(this.dataModel.Fungibles.FungibleRicKey).ConfigureAwait(false);
                ...
                foreach (var columnKeyPair in columnResolutionKeys)
                {
                    foreach (UniqueKeyElement uniqueKeyElement in columnKeyPair.Value)
                    {
                        if (!distinctKeys.Contains(uniqueKeyElement))
                        {
                            ...
                            // Make sure we only lock it once.
                            distinctKeys.Add(uniqueKeyElement);
                        }
                    }
                }

                //                    HashSet<Fungible> lockedFungibles = new HashSet<Fungible>();
                //                    HashSet<Issuer> lockedIssuers = new HashSet<Issuer>();
                ...
                foreach (var columnKeyPair in columnResolutionKeys)
                {
                    foreach (UniqueKeyElement uniqueKeyElement in columnKeyPair.Value)
                    {
                        ...
                        if (!distinctTables.Contains(tableElement))
                        {
                            statements.Add(
                                SyntaxFactory.LocalDeclarationStatement(
                                    ...
                                    .WithVariables(
                                        SyntaxFactory.SingletonSeparatedList<VariableDeclaratorSyntax>(
                                            SyntaxFactory.VariableDeclarator(
                                                SyntaxFactory.Identifier($"locked{tableElement.Name.ToPlural()}"))
                                            .WithInitializer(
                                                SyntaxFactory.EqualsValueClause(
                                                    SyntaxFactory.ObjectCreationExpression(
                                                        SyntaxFactory.GenericName(
                            ...
                            // Make sure we only add one hash set per table.
                            distinctTables.Add(uniqueKeyElement.Table);
                        }
                    }
                }
                ...
                statements.Add(
                    SyntaxFactory.ForEachStatement(
                        ...
                        SyntaxFactory.Identifier("jObject"),
                        SyntaxFactory.IdentifierName("jArray"),
                        SyntaxFactory.Block(this.ProcessExternalKeysBlock(columnResolutionKeys))));
        ...
        private List<StatementSyntax> TryBlock
        {
            get
            {
                ...
                // This provides the ability to reference parent records symbolically.
                if (this.tableElement.ParentKeys.Any())
                {
                    ...
                    statements.Add(
                        SyntaxFactory.UsingStatement(
                             SyntaxFactory.Block(this.ResolveParentRecordBlock))
                         .WithDeclaration(
                             SyntaxFactory.VariableDeclaration(
                                 SyntaxFactory.IdentifierName(
                                     SyntaxFactory.Identifier(
